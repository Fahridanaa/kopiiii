...
class ObjectDetector {
  static const modelPath = 'assets/best.tflite';
    ...
    try {
      ...
      for (var i = 0; i < 8400; i++) {
        ...
        if (maxScore > confidenceThreshold) {
          // Get coordinates and normalize them (0-1)
          final x = results[0][i] / inputSize; // Normalize by dividing by 640
          final y = results[1][i] / inputSize;
          final w = results[2][i] / inputSize;
          final h = results[3][i] / inputSize;

          final bbox = [
            x - w / 2, // normalized xmin
            y - h / 2, // normalized ymin
            x + w / 2, // normalized xmax
            y + h / 2, // normalized ymax
          ];
